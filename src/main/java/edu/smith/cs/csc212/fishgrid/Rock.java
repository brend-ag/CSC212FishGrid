...
/**
 * This is not <a href="https://en.wikipedia.org/wiki/Dwayne_Johnson">the Rock</a>, but a Rock. <-wow
 * @author jfoley
 */
public class Rock extends WorldObject {
	...
	public Rock(int color, World world) {
		super(world);
		this.color = color;
		/* All WorldObjects have 
		 * a ``rand`` available so you don't need to make one!!
		 */
	}
	 ...
	 */
	@Override
	public void draw(Graphics2D g) { //draws rock
		// use the right color in here... i think did?
		//this.color = getRColor();
		g.setColor(getRColor());
